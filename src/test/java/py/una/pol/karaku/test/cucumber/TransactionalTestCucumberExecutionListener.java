 ...
 * database during a test that is run with &#064;NotTransactional will
 * <strong>not</strong> be run within a transaction. Test methods that are not
 * annotated with either &#064;Transactional (at the class or method level) or
 ...
 * class-level {@link TransactionConfiguration &#064;TransactionConfiguration}
 * and method-level {@link Rollback &#064;Rollback} annotations.
 ...
 * support for methods annotated with {@link BeforeTransaction
...
public class TransactionalTestCucumberExecutionListener extends
		AbstractTestExecutionListener {

	private static final Log logger = LogFactory
			.getLog(TransactionalTestCucumberExecutionListener.class);
	...
	private final Map<Class, TransactionContext> transactionContextCache = Collections
			.synchronizedMap(new IdentityHashMap<Class, TransactionContext>());
	 ...
	 * <p>
	 * {@link AfterTransaction &#064;AfterTransaction methods} are guaranteed to
	 * be invoked even if an error occurs while ending the transaction.
	 */
	@Override
	public void afterTestClass(TestContext testContext) throws Exception {

		TransactionContext txContext = this.transactionContextCache
				.remove(testContext.getTestClass());
	...
	protected void runAfterTransactionMethods(TestContext testContext)
		...
		List<Method> methods = getAnnotatedMethods(testContext.getTestClass(),
				AfterTransaction.class);
		for (Method method : methods) {
			try {
				if (logger.isDebugEnabled()) {
					logger.debug("Executing @AfterTransaction method ["
							+ method + "] for test context [" + testContext
							+ "]");
				}
				method.invoke(testContext.getTestInstance());
	 ...
	 * @throws Exception
	 *             if an error occurs while retrieving the transaction manager
	 */
	private void startNewTransaction(TestContext testContext,
			TransactionContext txContext) throws Exception {

		txContext.startTransaction();
		++this.transactionsStarted;
		if (logger.isDebugEnabled()) {
			logger.debug("Start new Transaction [" + testContext.getTestClass()
	...
	private void endTransaction(TestContext testContext,
		...
		txContext.endTransaction(true);

		if (logger.isDebugEnabled()) {
			logger.debug("End Transaction [" + testContext.getTestClass()
					+ "] for test context [" + testContext + "]");
		}
	}

	/**
	 * Get the {@link PlatformTransactionManager transaction manager} to use for
	 * the supplied {@link TestContext test context}.
	 * 
	 * @param testContext
	 *            the test context for which the transaction manager should be
	 *            retrieved
	...
	protected final PlatformTransactionManager getTransactionManager(
		...
		String tmName = retrieveConfigurationAttributes(testContext)
				.getTransactionManagerName();
		try {
			return testContext.getApplicationContext().getBean(tmName,
					PlatformTransactionManager.class);
		} catch (BeansException ex) {
			if (logger.isWarnEnabled()) {
				logger.warn(
						"Caught exception while retrieving transaction manager with bean name ["
								+ tmName + "] for test context [" + testContext
								+ "]", ex);
			}
			throw ex;
		}
	 ...
	 * 
	 * @param testContext
	 *            the test context for which the default rollback flag should be
	 *            retrieved
	 * @return the <em>default rollback</em> flag for the supplied test context
	 * @throws Exception
	 *             if an error occurs while determining the default rollback
	 *             flag
	 */
	 ...
	 * 
	 * @param testContext
	 *            the test context for which the rollback flag should be
	 *            retrieved
	...
	protected final boolean isRollback(TestContext testContext)
		...
		Rollback rollbackAnnotation = testContext.getTestMethod()
				.getAnnotation(Rollback.class);
		if (rollbackAnnotation != null) {
			boolean rollbackOverride = rollbackAnnotation.value();
			if (logger.isDebugEnabled()) {
				logger.debug("Method-level @Rollback(" + rollbackOverride
						+ ") overrides default rollback [" + rollback
						+ "] for test context [" + testContext + "]");
			}
