...
final class SelectItemsIterator implements Iterator<SelectItem> {
		...
		if (kid instanceof UISelectItem) {
			...
			if (value != null) {
				if (value instanceof SelectItem) {
					...
					items = new IterableItemIterator(ctx, (UISelectItems) kid,
							(Iterable<?>) value);
	...
	private abstract static class GenericObjectSelectItemIterator implements
		...
		private static final class GenericObjectSelectItem extends SelectItem {
			...
			private void updateItem(FacesContext ctx, Object value) {
				...
				Map<String, Object> reqMap = ctx.getExternalContext()
						.getRequestMap();
	...
	private static final class ArrayIterator extends
		...
		private ArrayIterator(FacesContext ctx, UISelectItems sourceComponent,
				Object array) {
